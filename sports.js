...
function displayNews() {
    document.getElementById("headMain").innerHTML = ` <h3 id="headMain" class="headline-1">Top Sports News by <span class="badge bg-secondary ba-clr"> iTechNews</span></h3>`
    let sports1 = fetch(sportsNews)
    sports1.then((response) => {
        return response.json()
    }).then((response) => {
        AllData = response.articles;
        let newsHtml = ``
